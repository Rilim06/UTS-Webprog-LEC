                ...
                <div class="flex items-center mt-4">
                    <form method='POST' action='pesan_process.php'>
                        ...
                        <button type='button' class='bg-blue-500 text-white px-2 py-1 rounded'
                            data-item='item-count-" . $drinkID . "-decrement'>-</button>
                        <input type='text' id='item-count-" . $drinkID . "' name='item_count' value='1' readonly
                            class='w-10 text-center mx-2'>
                        <button type='button' class='bg-blue-500 text-white px-2 py-1 rounded'
                            data-item='item-count-" . $drinkID . "-increment'>+</button>
                        <button type='submit' value='add_drink' name='pesan'
                ...
                if ($_SESSION["username"] == 'admin') {
                    echo "
                        <div class='flex items-center mt-4'>
                            ...
                            <form method='POST' action='delete_menu.php'>
                                <input type='hidden' name='id' id='food-id-del' value=''>
                                <button type='submit' class='bg-blue-500 text-white px-2 py-1 rounded' value='drink' name='delete'>Delete</button>
                            </form>
                        </div>
                    ";
                } else {
                    echo "
                        <div class='flex items-center mt-4'>
                            ...
                            <input type='hidden' name='id' id='food-id-del' value=''>
                                <input type='hidden' name='id' id='food-id' value=''>
                                <button type='button' class='bg-blue-500 text-white px-2 py-1 rounded' data-item='item-count-" . $drinkID . "-decrement'>-</button>
                                <input type='text' id='item-count-" . $drinkID . "' name='item_count' value='1' readonly class='w-10 text-center mx-2'>
                                <button type='button' class='bg-blue-500 text-white px-2 py-1 rounded' data-item='item-count-" . $drinkID . "-increment'>+</button>
                                <button type='submit' value='add_drink' name='pesan' class='ml-auto mt-2 bg-green-500 text-white px-4 py-2 rounded'>Add to Cart</button>
                            </form>
                        </div>
                    ";
                }
    ...
    if (foodImages.length > 0) {
        ...
        foodImages.forEach(image => {
            image.addEventListener('click', () => {
                ...
                const drinkID = image.getAttribute('id');
                const drinkIDD = image.getAttribute('id');
